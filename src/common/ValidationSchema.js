...
const ValidationSchema = Yup.object().shape({
  title: Yup.string().min(2, 'Title too Short!').max(200, 'Title too Long!').required('Title Required'),

  price: Yup.number().min(1, 'Number too Short!').max(100000, 'Number is up to 100000!').required('Price Required'),

  description: Yup.string().min(6, 'Description too Short!').max(2000, 'Description too Long!').required('Description Required'),
